...
def test_secret_config_reports_missing_required_variables() -> None:
    with pytest.raises(SecretConfigError) as exc_info:
        SecretConfig.from_env(
            ["OPENROUTER_API_KEY", "SEARCH_PROVIDER_API_KEY"],
            environ={},
        )
...
def test_secret_config_reads_environment_values_without_repr_leaks(
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", model_secret)
    ...
    assert config.value("OPENROUTER_API_KEY") == model_secret
...
def test_approved_secret_write_creates_env_local_with_redacted_result(tmp_path) -> None:
    ...
    assert env_path.read_text() == "OPENROUTER_API_KEY=approved-secret-value\n"
    assert result.written_variables == ("OPENROUTER_API_KEY",)
...
def test_redact_secret_values_scrubs_raw_values_from_output() -> None:
    raw_secret = "sk-or-abcdefghijklmnopqrstuvwxyz1234567890"

    output = redact_secret_values(
        f"Request failed with OPENROUTER_API_KEY={raw_secret}.",
        [raw_secret],
    )
